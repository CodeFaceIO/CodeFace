  ...
  if (folderType === 'inode/directory') {
    ...
    folderReader.readEntries(function (entries) {
      ...
      for (let i = 0; i < folderEntries.length; i++) {
        ...
        // check if folder entry is json
        if (folderEntryType === 'application/json') {
          // check if folder entry is configure.json
          if (folderEntryName === 'configure.json') {
            // check if folder entry has correct json structure
            const folderEntryReader = folderEntry.createReader();
            folderEntryReader.readEntries(function (entries) {
              // read entries of folder entry
              const folderEntryEntries = entries;
              for (let i = 0; i < folderEntryEntries.length; i++) {
                const folderEntryEntry = folderEntryEntries[i];
                const folderEntryEntryName = folderEntryEntry.name;
                const folderEntryEntryPath = folderEntryEntry.path;
                const folderEntryEntryType = folderEntryEntry.type;
