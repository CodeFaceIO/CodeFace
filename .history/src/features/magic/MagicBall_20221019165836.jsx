...
const MagicBall = () => {
  ...
  return (
    <div className={`${styles.magic_box}`} style={{ transform: `translateX(${translateY}px)` }}>
      ...
      {!isOpen ? (
        <>
          <div className={`d-flex align-items-start ${styles.header_container}`}>
            <div className={`${styles.user_profile}`}>
              <div></div>
            </div>
            <h1 className={`${styles.user_username}`}>Username</h1>
            <div className={`${styles.user_socials_container}`}>
              <AiFillGithub className={`${styles.user_socials}`} />
              <AiFillInstagram className={`${styles.user_socials}`} />
              <AiFillTwitterCircle className={`${styles.user_socials}`} />
              <img src={AiDailyIcon} alt="daily dev view" className={`${styles.user_socials_}`} />
            </div>
          </div>
        </>
      ) : null}
